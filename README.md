...
runs the scanner, which pre-renders one static HTML page per channel, and the
published site is plain HTML and CSS. Row expansion uses native
`<details>`/`<summary>`, channel switching is a plain link row, and the help
text is a normal section on each page.
...
src/render.ts          server-side rendering of the no-JS HTML pages
site/                  generated pages: index.html + one page per target, style.css
...
`site/index.html` and `site/<target>.html` are generated by the scanner on
every run and committed so Pages can serve them directly.

## Running the scanner

```bash
bun src/scanner.ts --config config.json --data site/state.json
```
